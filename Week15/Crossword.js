...
var dropdownHeight = 35;
var dropdownX = 340;
var dropdownY = 575;

// Clues
var clues = [
   "1 Across: Airer of '48 Hours' and '60 Minutes', 1 Down: At the home of: Fr.",
  "2 Down: Benjamin Netanyahu's nickname",
  "3 Down: 'Get real for heaven's sake!'",
  "4 Across: Inscribe, as on a trophy, 4 Down: Airline that doesn't schedule flights on Shabbat",
  ...
  "27 Across: Exercises that work the glutes, quads and abs, 27 Down: Pinch pennies",
  ...
  "47 Across: Star of Broadway's 'Fiddler on the Roof', 47 Down: Clearasil target, in slang",
  "48 Down: Went 'Hello...ello...llo...lo...'",
  "49 Down: Keep on the shelves",
  "50 Across: Tilted, as printed letters",
  "51 Down: True-blue",
  "52 Across: To the _ degree",
  "53 Across: 'Forget about it!'... or a clue to the starts of 20-, 27- and 47-Across",
...
function crosswordTextBoxes(i, j, placeholderText) {
  ...
  textBoxes[i][j].position(100 + j * gridSize + 2, 100 + i * gridSize + 2);
  textBoxes[i][j].size(gridSize - 7.2, gridSize - 5);
...
function drawTitleText() {
  textStyle(BOLD);
  textSize(40);
  text('Crossword', 20, 50);
  textSize(16);
  textStyle(NORMAL);
  text('based off The New York Times crossword puzzle', 320, 30);
  text('by Andrea Carla Micheals', 468, 45);
  text('no. 0302', 580, 60);
  textSize(32);
  text('Clues:', 250, 603);
}
...
function displayClue(selectedValue) {
  ...
  if (selectedValue) {
    ...
    if (index <= 0 && index < clues.length) {
      // Display the clue for the selected value
      text(clues[index], 25, 635);
